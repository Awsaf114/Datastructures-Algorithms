...
void Graph::Floyd_Warshall(){
    ...
    if(D[s][t] == INFINITY || D[s][t] == 0){
        cout<<"Path Does not exist"<<endl;
    }
    /// Gives wrong output
    if( D[s][u]!= INFINITY && D[v][t] != INFINITY ){
        int pathW = D[s][u] + D[u][v] + D[v][t]; /// Use weight of the vertex, NOT D.
        if(D[s][t] < pathW ){
            cout<<"\nNo , Decrease By :\n"<<(pathW - D[s][t])<<endl;
        }
        else{
...
//        if(s==t)
//        {
//            cout<<t+1<<" Same Node"<<endl;
//        }
//        if(pi[s][t] == INFINITY){
//            cout<<t+1<<" No Path"<<endl;
//        }
//        else{
//            int k = t;
//            while(pi[s][k] != s+1){
