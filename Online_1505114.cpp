...
void ArrayList::changenode(int w, int pos){
    list[pos].weight = w;
}
...
public:
    ...
    void findcyclevisit(int source);
};

void Graph::changeEdge(int u , int v, int w){
    int weight;
//cout<<endl<<parent[v]<<endl;

    if(v == parent[u]){
        weight =  adjList[v].getItem(adjList[v].searchItem(parent[v])).weight;
    }
    else{
        weight =  adjList[u].getItem(adjList[u].searchItem(parent[u])).weight;
    }
   // cout<<endl<<weight<<endl;
    if(w<weight){
        adjList[u].changenode(w , adjList[u].searchItem(parent[u]));
        printMST();
    }
    else{
        //findcycle(u);
...
void Graph::findcyclevisit(int source){
    if(color[source] == 3){
        return;
    }
...
void Graph::findcycle(int source)
{
    count = 1;
    for(int i = 0; i<nVertices ; i++){
        color[i] = WHITE;
    }
    findcyclevisit(source);
