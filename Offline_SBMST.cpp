...
public:
    ...
    int MST_Prim(int root);
    void SBMST();
    int printMST();
    void MAXfill_visit(int u , int w);
    void dfs_MAXfill(int source);

};


int Graph::MST_Prim( int root){
    MinHeap heap;
...
void Graph::SBMST(){
    ...
    int w[nVertices] , MSTEdges[nVertices];
    w[0] = MST_Prim(0);
    ...
    for(int i = 1 ; i<nVertices ; i++){
        ...
        int weight = removeEdge(i , MSTEdges[i]);
        dfs(i);
        if(!connected){
            addEdge( i , MSTEdges[i] , weight);
            w[i] = INFINITY;
        }
        else{
            w[i] = MST_Prim(0);
    ...
    int min = INFINITY , idx;
    for(int i = 1 ; i <nVertices ; i++){
        if(w[i]< min){
            min = w[i];
            idx = i;
      //      printf("%d - (%d , %d)", min , i , parent[i]);

        }
    }
    int weight = removeEdge(idx , MSTEdges[idx]);
    cout<<endl<<"Second best MST is :"<<endl;
    MST_Prim(0);

    addEdge(idx , MSTEdges[idx] , weight);
}
...
{
    if(u<0 || v<0 || u>=nVertices || v>=nVertices) return NULL_VALUE; //vertex out of range
    this->nEdges-- ;
    int pos = adjList[u].searchItem(v);
    GraphNode item = adjList[u].getItem(pos);
